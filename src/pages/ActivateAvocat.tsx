...
const ActivateAvocat = () => {
  ...
  useEffect(() => {
    const loadInvitation = async () => {
      ...
      try {
        const { data, error } = await supabase.functions.invoke("activate-avocat", {
  ...
  const handleActivate = async (e: React.FormEvent) => {
    ...
    if (password.length < 10) {
      toast({
        ...
        description: "Utilisez au moins 10 caractères.",
        variant: "destructive",
      });
      return;
    }
    ...
    try {
      ...
      if (signInError) {
        toast({
          title: "Compte activé",
          description: "Le compte est créé. Connectez-vous avec votre email et votre mot de passe.",
        });
      ...
      toast({
        title: "Compte avocat activé",
        description: "Vous pouvez accéder à votre espace avocat.",
      });

      navigate("/avocat", { replace: true });
    } catch (err: unknown) {
      toast({
    ...
    <div className="fixed inset-0 bg-background flex flex-col items-center justify-center p-6">
      <div className="w-full max-w-[440px]">
        <div className="text-center mb-8">
          ...
          <p className="text-muted-foreground text-sm">Activation d’un compte avocat</p>
        </div>

        <div className="bg-panel border border-border rounded-xl p-6">
          <div className="mb-5 space-y-1.5">
            <div className="text-sm"><strong>Nom :</strong> Me {invitation.prenom} {invitation.nom}</div>
            <div className="text-sm"><strong>Identifiant :</strong> {invitation.email}</div>
            <div className="text-sm"><strong>Barreau :</strong> {invitation.barreau}</div>
            <div className="text-sm"><strong>Capacité :</strong> {invitation.capacite_max} dossier(s)</div>
          ...
          <form onSubmit={handleActivate} className="space-y-4">
            ...
            <p className="text-[0.7rem] text-muted-foreground">
              Après activation, vous vous connecterez avec votre email comme identifiant.
            </p>
            ...
            >
              {loading ? "Activation…" : "Activer mon compte avocat"}
            </button>
          </form>
        </div>
      </div>
