...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'dossiers_option_choisie_check'
  ) THEN
    ALTER TABLE public.dossiers
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'payments_option_choisie_check'
  ) THEN
    ALTER TABLE public.payments
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'dossiers_validation_juridique_mode_check'
  ) THEN
    ALTER TABLE public.dossiers
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'dossiers_validation_juridique_status_check'
  ) THEN
    ALTER TABLE public.dossiers
      ADD CONSTRAINT dossiers_validation_juridique_status_check
      CHECK (validation_juridique_status IN (
        'non_evaluee', 'validee_automatique', 'a_verifier_avocat', 'validee_avocat', 'bloquee'
      ));
  END IF;
END $$;

CREATE INDEX IF NOT EXISTS idx_dossiers_option_choisie ON public.dossiers(option_choisie);
CREATE INDEX IF NOT EXISTS idx_payments_option_choisie ON public.payments(option_choisie);
CREATE INDEX IF NOT EXISTS idx_dossiers_validation_juridique_status ON public.dossiers(validation_juridique_status);